...
test("Center Compose는 공유 Redis와 색상별 세 worker를 외부 포트 없이 정의한다", () => {
  ...
  assert.match(compose, /node", "dist\/workers\/pushWorker\.js", "--healthcheck/);
  assert.match(compose, /node", "dist\/workers\/accountDeletionWorker\.js", "--healthcheck/);
  assert.match(compose, /DB_POOL_MAX: "2"/);
  assert.match(compose, /source: firebase_service_account/);
  assert.match(compose, /x-account-deletion-worker-common:[\s\S]*source: apple_signin_private_key/);
...
test("Stage Compose는 API, Redis와 세 worker를 저장소 정본으로 정의한다", () => {
  ...
  assert.match(compose, /file: \.\/secrets\/firebase\.json/);
  assert.match(compose, /name: shiftmate_stage_internal[\s\S]*external: true/);
  assert.match(compose, /env_file:\n\s+- \.env\n\s+- feature-flags\.env/);
...
test("Apple secret은 feature flag 없이 API와 탈퇴 worker에 mount되고 배포 전에 검증된다", () => {
  ...
  assert.match(center_base, /x-api-common:[\s\S]*source: apple_signin_private_key/);
  assert.match(stage_base, /shiftmate_stage_api:[\s\S]*source: apple_signin_private_key/);
  assert.match(
    center_base,
  ...
  assert.doesNotMatch(script, /readBooleanEnvironmentValue|APPLE_AUTH_ENABLED/);
...
test("배포는 Redis, Stage API/worker, Center API/worker, Nginx 순서를 지킨다", () => {
  ...
  const stage_deploy = script.indexOf("Deploying Stage API and workers", stage_redis);
  ...
  assert.ok(center_push_worker_health < center_account_deletion_worker_health);
...
test("Stage 배포 설정은 API, 세 worker, Redis를 각각 식별한다", () => {
  ...
  assert.match(example, /^STAGE_WORKER_SERVICE=shiftmate_stage_cache_worker$/m);
  assert.match(example, /^STAGE_PUSH_WORKER_SERVICE=shiftmate_stage_push_worker$/m);
  assert.match(
    example,
  ...
  assert.match(script, /CENTER_FIREBASE_SECRET_FILE=.*secrets\/firebase\.json/);
  assert.match(script, /STAGE_FIREBASE_SECRET_FILE=.*secrets\/firebase\.json/);
...
test("고정 launcher는 exact main checkout의 배포 엔진과 Compose/config만 root 번들로 실행한다", () => {
  ...
  const sudoers = readRepositoryFile("deploy/sudoers/github-runner-shiftmate");
  ...
  assert.match(launcher, /deploy image SHA must match the checked-out source SHA/);
  ...
  assert.match(launcher, /rev-parse "\$\{source_sha\}:\$\{repository_path\}"/);
  assert.match(launcher, /verifySourceFile deploy\/shiftmate-deploy 100755/);
  assert.match(launcher, /verifySourceFile deploy\/compose\.production\.yaml 100644/);
  assert.match(launcher, /verifySourceFile deploy\/compose\.stage\.yaml 100644/);
  assert.match(launcher, /verifySourceFile deploy\/config\/feature-flags\.production\.env 100644/);
  assert.match(launcher, /verifySourceFile deploy\/config\/feature-flags\.stage\.env 100644/);
...
test("검증된 main Compose와 feature flag config를 원자 설치하고 실패 시 복원한다", () => {
  ...
  assert.match(script, /Center Compose source must be owned by root:root with mode 600/);
  assert.match(script, /Stage Compose source must be owned by root:root with mode 600/);
  assert.match(script, /Center feature flags source must be owned by root:root with mode 600/);
  assert.match(script, /Stage feature flags source must be owned by root:root with mode 600/);
  assert.match(script, /--project-directory "\$APP_DIR"[\s\S]*--profile blue[\s\S]*--profile green[\s\S]*config --quiet/);
  assert.match(script, /--project-directory "\$STAGE_APP_DIR"[\s\S]*--file "\$STAGE_COMPOSE_SOURCE"[\s\S]*config --quiet/);
  assert.match(script, /temp_file="\$\(mktemp "\$\{target_file\}\.XXXXXX"\)"[\s\S]*install -o root -g root -m 0644[\s\S]*mv -f "\$temp_file" "\$target_file"/);
  ...
  assert.match(script, /cp --preserve=mode,ownership "\$COMPOSE_FILE" "\$CENTER_COMPOSE_BACKUP"/);
  assert.match(script, /cp --preserve=mode,ownership "\$STAGE_COMPOSE_FILE" "\$STAGE_COMPOSE_BACKUP"/);
  assert.match(
    script,
...
test("main push 배포와 수동 rollback은 self-hosted exact checkout을 launcher로 실행한다", () => {
  ...
  const deploy_job = deploy_workflow.slice(deploy_workflow.indexOf("  deploy:"));
  ...
  assert.match(deploy_job, /permissions:\n\s+contents: read\n\s+packages: read/);
  ...
  assert.match(deploy_job, /sudo \/usr\/local\/sbin\/shiftmate-deploy-launcher/);
...
test("CI는 cache/push/회원 탈퇴 PostgreSQL·Redis 통합 테스트를 이미지 빌드 전에 실행한다", () => {
  ...
  const push_integration_test = workflow.indexOf("run: npm run test:push-integration");
...
test("npm lockfile은 CI clean install에 필요한 optional dependency metadata를 완성한다", () => {
  ...
  const incomplete_entries = package_entries
    .filter(([package_path, metadata]) => package_path && !metadata.link && !metadata.version)
